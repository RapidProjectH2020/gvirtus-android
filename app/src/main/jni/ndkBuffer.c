...
Java_eu_project_rapid_gvirtus_gvirtus4a_Buffer_00024Helper_prepareFloat(JNIEnv *env,
                                                                   jobject instance,
                                                                   jfloatArray arr) {
    jstring result=NULL;
    size_t i;
...
JNIEXPORT jstring JNICALL
Java_eu_project_rapid_gvirtus_gvirtus4a_Buffer_00024Helper_preparePtxSource(JNIEnv *env,
                                                                       jobject instance,
                                                                       jstring ptxSource_,
                                                                       jlong size) {
    size_t i,j;
    ...
    const char *nativeString = (*env)->GetStringUTFChars(env, ptxSource_, 0);
...
printf("name printed as %%x 3 is %02X\n",nativeString[3] & 0xff);*/
    ...
    (*env)->ReleaseStringUTFChars(env, ptxSource_, nativeString);
    result = (*env)->NewStringUTF(env,msg);
    free(msg);
...
JNIEXPORT jstring JNICALL
Java_eu_project_rapid_gvirtus_gvirtus4a_Buffer_00024Helper_prepareSingleByte(JNIEnv *env,
